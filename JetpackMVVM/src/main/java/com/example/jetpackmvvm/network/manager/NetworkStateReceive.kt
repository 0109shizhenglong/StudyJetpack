        ...
        if (intent.action == ConnectivityManager.CONNECTIVITY_ACTION) {
            if (!isInit) {
                if (!NetworkUtil.isNetworkAvailable(context)) {
                    ...
                    NetworkStateManager.instance.mNetwork.value?.let {
                        if (it.isSuccess) {
                            //没网
                            NetworkStateManager.instance.mNetwork.value = NetState(isSuccess = false)
                        }
                        return
                    }
                    NetworkStateManager.instance.mNetwork.value = NetState(isSuccess = false)
                    ...
                    NetworkStateManager.instance.mNetwork.value?.let {
                        if (!it.isSuccess) {
                            //有网络了
                            NetworkStateManager.instance.mNetwork.value = NetState(isSuccess = true)
                        }
                        return
                    }
                    NetworkStateManager.instance.mNetwork.value = NetState(isSuccess = true)
