...
import com.example.jetpackmvvm.state.parseResult
...
fun <T> BaseVmActivity<*>.parseState(
    ...
    when (resultState) {
        ...
        is ResultState.Success -> {
            dismissLoading()
            onSuccess?.run { this(resultState.data) }
        }
...
/**
 *显示页面状态，这里有个技巧，成功回调在第一个，其后两个带默认值的回调可省略
 */
...
/**
 * 请求结果过滤，判断请求服务器请求结果是否成功，不成功否则会抛出异常
 */
